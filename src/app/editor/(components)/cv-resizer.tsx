  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      ...
      if(newLeftPanelWidth === 20) {
        setPanel(prev => ({ ...prev, width: 50, showLeft: false, isDragging: false }));
        return;
      }
  
      if(newLeftPanelWidth === 80) {
        setPanel(prev => ({ ...prev, width: 50, showRight: false, isDragging: false }));
        return;
      }
    ...
    >
      ...
      <div className={classNames("absolute top-1/2 transform -translate-y-1/2 flex flex-col space-y-2", {
        ...
        {panel.showRight && (
          <Button
            size="icon"
            onClick={() => setPanel(prev => ({ ...prev, showLeft: !panel.showLeft }))}
          >
            {panel.showLeft ? <ChevronLeft className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
          </Button>
        )}
        {panel.showLeft && (
          <Button
            size="icon"
            onClick={() => setPanel(prev => ({ ...prev, showRight: !panel.showRight }))}
          >
            {panel.showRight ? <ChevronRight className="h-4 w-4" /> : <ChevronLeft className="h-4 w-4" />}
          </Button>
        )}
      </div>
    </Separator.Root>
